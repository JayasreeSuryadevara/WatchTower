...
const newsKey = require('../../config/keys_dev').NewsApiKey;
...
export function headlineNews() {
  const url = `https://newsapi.org/v2/everything?q="headline"&language=en&domains=wsj.com,nytimes.com,cnbc.com,foxbusiness.com,seekingalpha.com,businessinsider.com&pageSize=50&sortBy=publishedAt&apiKey=${newsKey}`
  return (
    axios.get(url).then(res => res.data.articles)
  )
}
